// Keeps this trigger proposal's review separate from the immutable 300-script receipt.
export function createTriggerDecisions(config, lines, currentMoment) {
  ...
  function receipt() {
    return { id: config.id, version: config.version, catalogHash: config.catalogHash,
      reviewOnly: true, scope: 'New trigger proposal only; no generation or gameplay authorization',
      policy: config.timing, decisions: config.moments.map((m, index) => ({
        triggerId: `T${String(index + 1).padStart(2, '0')}`, group: m.id, title: m.title,
  ...
  function save(decision, refresh = true) {
    ...
    try {
      ...
      el('save-status').textContent = qa ? 'QA mode: test choices are not saved.' : 'Trigger decisions saved in this browser. The 300-script receipt is unchanged.';
  ...
  function prepare() {
    ...
    el('reply').value = [`Leo trigger mockup v1: ${t.yes} yes, ${t.no} no, ${t.pending} pending.`,
