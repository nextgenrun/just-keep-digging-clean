...
The promoted complex SIDE/UP subset stays on the production 160-bone Survival skeleton and V4 material treatment. The prior native Jab/Cross and Blender upward dig now participate as short-reach canonical clips; DOWN, diagonals and Thunder retain their existing routing. `AnimationTileClearanceResolver` filters only presentation choices in the main world and compact caves: wide arcs require their authored neighboring cells to be open, moving input retains rooted legacy strikes, and a canonical clip is always available. It never changes the selected mining cell, physics body, contact timing, or tile authority. `?animationClearance=0` restores the unfiltered sequence. `PlayerKinematicMotionSystem` exposes signed post-collision velocity for shared locomotion transitions and flight banking. The measured 31x75 body and one-cell contact perimeter remain authoritative; projected limb markers remain diagnostic and bounded visual-alignment evidence rather than gameplay reach authority.
