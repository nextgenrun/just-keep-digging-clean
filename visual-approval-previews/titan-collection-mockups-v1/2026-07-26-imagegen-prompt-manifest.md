...
## 05 — Production Titan Walk Plinth

Outputs:

- Chroma source:
  `sprites/backgrounds/titan-discoveries-v1/sources/2026-07-26-titan-walk-plinth-chroma-v1.png`
- Transparent alpha master:
  `sprites/backgrounds/titan-discoveries-v1/sources/2026-07-26-titan-walk-plinth-alpha-v1.png`
- Normalized 512x320 runtime sprite:
  `sprites/backgrounds/titan-discoveries-v1/titan-walk-plinth-v1.png`

Input:

1. `2026-07-26-03-surface-titan-walk-v1.png` — approved dark-slate, ancient
   root, brass-trim, and dormant-rune material language.

Prompt:

```text
Use case: transparent game asset extraction
Asset type: one production-ready 2D Phaser environment prop
Primary request: Using the referenced UNDERSTAR Surface Titan Walk mockup,
create one isolated empty Titan Walk display plinth. Preserve its low
front-facing side-view shape, dark obsidian slate, restrained antique-brass
trim, ancient roots curling around the base, and a faint dormant rune recess
where a small Titan projection will stand. The plinth must feel old, premium,
and physically grounded, but remain non-interactive scenery.
Composition: exactly one complete plinth centered in frame, fully visible with
generous clear space on every edge, symmetrical enough to repeat in a long
25-position memorial, broad stable base, no cropped roots or trim.
Background: perfectly flat saturated chroma magenta, one uniform color, with no
shadow or glow contaminating the background.
Lighting/style: polished cinematic painterly 2D game prop matching UNDERSTAR,
cool slate body, subtle warm bronze edge light, readable at roughly one game
tile wide.
Constraints: no Titan creature, no character, no text, no numbers, no plaque,
no UI, no environment, no separate pieces, no cast shadow outside the object,
no transparency checkerboard, no watermark.
```

The chroma source was converted with the bundled ImageGen
`remove_chroma_key.py` helper. `ai-tools/2026-07-26-build-titan-sprites.py`
then alpha-crops, validates, and fits the master into the bounded runtime
canvas.
