...
> Superseded by the approved fixed-center Piskel v3 package under
> `sprites/vehicles/arc-core-v3`. These whole-body boards remain comparison and
> rejection evidence only and are no longer loaded by gameplay or the sandbox.
...
- These historical boards do not replace production textures.
- Pointed Small Arc `v1` is explicitly rejected and no longer loaded.
- The current production package is `sprites/vehicles/arc-core-v3`, registered
  by `values/arcCoreVisuals.sprite.json`.

## Direction

- Small Arc round `v2` and Omega `v1` are retained as identity references for
  the fixed-body masters in the approved Piskel package.
- Current motion uses separate gyro/sigil layers rather than four whole-body
  idle and dig poses.
- Current dig and enter/exit presentation uses Piskel-authored beam, impact,
  cyan-cloud, and violet-cloud sprite roles without procedural final artwork.
