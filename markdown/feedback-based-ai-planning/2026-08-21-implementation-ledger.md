...
| P2 responsive Celestial tree | J03-02, A17-22, A20-02 | Branch focus and exact comparison implemented; browser proof pending | `42907be` | `CELESTIAL_BRANCH_FOCUS_CONTRACT_OK`; compact layouts auto-focus one branch, wide layouts retain the overview, authored branch tabs support manual focus, Escape returns to overview, connectors follow the active topology, and the detail dossier compares inactive/active state with the configured effect | Responsive focus plus slice revert |
...
| P2 merchant/deep presentation authority | A17-14, J03-13 | Prompt anchor and deep feature authority implemented; browser proof pending | `e518e44` | `MERCHANT_PROMPT_ANCHOR_CONTRACT_OK`; prompt position is derived from the merchant ground surface with a compact-sprite compatibility clamp. Deep-world backdrop smoke and both player-light contracts pass with injected Level Two capability authority and upper-body light centering | Values/slice revert |
...
| P3 underground particle identity | J03-12, J06-06 | Implemented; browser/performance proof pending | next slice commit | `AMBIENT_PARTICLE_BAND_CONTRACT_OK`; four presentation-only depth identities, exact active-band diagnostics, FPS/live-count gates retained, immediate clearing through explicit building-occupancy authority, and no surface Town particles because emission begins underground | Values/slice revert |
| P2 UI/audio/deep polish | Remaining raw and analysis IDs | In progress | — | — | Presentation flags plus slice revert |
