...
The first-run progression states are Mine → Sell → Upgrade → Complete. Golden
Five owns the deeper Flight tutorial; the two systems must hand off rather than
show overlapping instructions.
...
The current outer save envelope is version 12. Relevant nested contracts are:

- Opening Flight state version 3, including `tutorialChoice` and idempotent
  artifact/cache flags;
- Hardcore mode version 2, including mode, armed state, lives, free revive,
