...
check("Landing requires a real airborne downward interval; spawn/teleport are silent", () => {
  ...
  for (const asset of CORE.hardFootsteps) sample.keys.add(asset.key);
  sample.scene.worldModel = { tileSize: 94, getTileType: () => 2 };
  const controller = { physicsBody: { x: 0, y: 0, w: 40, h: 80, vy: 0 }, isGrounded: () => grounded };
  ...
  assert.ok(CORE.hardFootsteps.some(asset => asset.key === sample.played[0].key));
