...
import {
  createArcCoreImagegenArtwork,
  drawArcCoreImagegenArtwork,
  hideArcCoreImagegenArtwork,
  preloadArcCoreImagegenArtwork,
} from "./arcCoreImagegenArtwork.js";
import {
...
const VIEW_W = 1280;
const VIEW_H = 720;
const WORLD_COLS = ARC_CORE_ANIMATION_REVIEW.stage.worldCols;
const WORLD_ROWS = ARC_CORE_ANIMATION_REVIEW.stage.worldRows;
...
function makeWorld() {
  ...
  for (let row = 0; row < WORLD_ROWS; row += 1) {
    ...
    for (let col = 0; col < WORLD_COLS; col += 1) {
      if (row >= ARC_CORE_ANIMATION_REVIEW.stage.normalBedrockRow) grid[row][col] = TILE.BEDROCK;
      else if (row === ARC_CORE_ANIMATION_REVIEW.stage.normalFloorRow) grid[row][col] = TILE.FLOOR;
...
function makeOmegaReviewWorld() {
  ...
  const stage = ARC_CORE_ANIMATION_REVIEW.stage;
...
const TankScene = new Phaser.Class({
  ...
  preload: function () {
    ...
    preloadArcCoreImagegenArtwork(this);
  },
  ...
  create: function () {
    ...
    this.arcImagegenArtwork = createArcCoreImagegenArtwork(this);
  ...
  createPanel: function () {
    ...
    this.smallArcButton = addButton("Small Arc — Needle", () => {
    ...
    this.arcArtworkButton = addButton("Arc Art: ImageGen", () => {
      this.useImagegenArcArt = !this.useImagegenArcArt;
      this.arcArtworkButton.textContent = this.useImagegenArcArt
        ? "Arc Art: ImageGen"
        : "Arc Art: Procedural";
      this.arcArtworkButton.classList.toggle("active", this.useImagegenArcArt);
      this.drawAll();
    }, true);
  ...
  drawOccluderAndDamage: function () {
    ...
    if (dig && !dig.broken) {
      const progress = this.getDigProgress(dig);
      if (isArcCoreAnimationReviewMode(this.characterMode)) {
        drawArcCoreAnimationReviewDamage(g, {
          mode: this.characterMode,
          targets: dig.targets || [dig.target],
          progress,
          direction: dig.direction,
          tileSize: TILE_SIZE,
          timeMs: this.time.now,
        });
      } else if (this.characterMode === CHARACTER_MODES.robotSphere) {
  ...
  updateInfo: function (dt) {
    ...
    this.infoEl.innerHTML = [
      "Small + Omega Arc Core Animation Review",
      ...
      ...(reviewMode
        ? [
          ...
          "Review only: production visuals unchanged",
        ]
        : []),
