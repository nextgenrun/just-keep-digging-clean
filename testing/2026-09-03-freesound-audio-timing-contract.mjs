...
import { FREESOUND_AUDIO as CFG, FREESOUND_AUDIO_ASSETS as ASSETS, FREESOUND_RUNTIME_ASSETS as ACTIVE } from "../values/freesoundAudio.js";
...
const consumers = {
  ...
  flightWhoosh: "FreesoundAudioDirector.observeFlight", coinPickup: "SoundSystem.playPurchase",
  coinReward: "SoundSystem.playCoinReward", uiClick: "SoundSystem.playUiClick", uiMechanical: "SoundSystem.playMenuOpen/playUiConfirm/playManualSave",
...
check("All active variants are reachable through bounded banks or stable site/episode selection", () => {
...
check("Actual mining dispatcher routes dirt, metal and crystal; Star destruction remains single", () => {
  ...
  for (const [type, role] of [[1, "mineEarth"], [3, "mineMetal"], [34, "crystalBreak"]]) {
    f.tick(10000); methods.playMineFeedbackAudio.call({ soundSystem: f.system }, { success: true, destroyed: role === "crystalBreak" }, type);
...
check("Dirt footsteps use the contacted floor; cave context cannot reuse overworld Stars or threats", () => {
  ...
  const controller = { physicsBody: body, isGrounded: () => true, abilities: { isFlying: () => false } };
  assert.equal(f.system.playFootstep({ controller, worldModel: { tileSize: 94, getTileType: () => 1 } }), null);
  body.vx = 130;
...
const report = { passed: true, checks, approved: Object.keys(ASSETS).length, active: Object.keys(ACTIVE).length, exportHash: FREESOUND_APPROVAL_HASH };
...
writeFileSync(new URL("testing/audio-review-2026-09-03/freesound-wiring-coverage.json", root), JSON.stringify({ ...report,
  assets: Object.values(ASSETS).map(asset => ({ id: asset.id, sourceRole: asset.role, role: ACTIVE[asset.id]?.role || asset.role, consumer: ACTIVE[asset.id] ? consumers[ACTIVE[asset.id].role] : null, path: asset.path, runtimeWired: Boolean(ACTIVE[asset.id]) })) }, null, 2) + "\n");
