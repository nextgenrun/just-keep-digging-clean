...
for (const card of cards) {
  ...
  assert.equal(region.variantBackwalls.length, 6);
  assert.ok(region.variantBackwalls.slice(0, 5).every(asset => asset.type === "image"));
  assert.equal(region.variantBackwalls[5].type, "video");
  assert.match(region.variantBackwalls[5].path, /\/biome-motion-v3\/.+-loop-v3\.mp4$/);
