...
function firstPaidRankPathCost(branch, nodeId) {
  ...
  const visit = (id, trail = new Set()) => {
    ...
    const rankCost = getCelestialTalentRankCost(node, 1);
    if (node.prerequisiteIds.length === 0) return rankCost;
...
for (const branch of talentConfig.branches) {
  ...
  const branchStarRankCost = branch.nodes.reduce((sum, node) => {
    let nodeCost = 0;
    for (let rank = 1; rank < CELESTIAL_TALENT_RANK_CONFIG.maxRank; rank += 1) {
      nodeCost += getCelestialTalentRankCost(node, rank);
    }
    return sum + nodeCost;
  }, 0);
  assert.equal(branchStarRankCost, 3675);
  assert.deepEqual(
    branch.completionNodeIds.map(nodeId => firstPaidRankPathCost(branch, nodeId)),
    [475, 475, 475],
    `${branch.id} capstones must offer equal-cost first-rank build choices`,
  );
...
const totalStarRankCost = allTalentNodes.reduce((sum, node) => {
  let nodeCost = 0;
  for (let rank = 1; rank < CELESTIAL_TALENT_RANK_CONFIG.maxRank; rank += 1) {
    nodeCost += getCelestialTalentRankCost(node, rank);
  }
...
assert.equal(getEarnedCelestialTalentPoints(35), allTalentNodes.length);
assert.equal(totalStarRankCost, 11025);
...
assert.equal(Object.keys(UPGRADES).length, 34);
...
console.log(JSON.stringify({
  talents: {
    branches: 3,
    nodes: 33,
    talentPointsToUnlockAll: 33,
    playerLevelToUnlockAll: 35,
    starRankPurchases: 66,
    starRankCost: totalStarRankCost,
    cheapestRankTwoCapstonePath: 475,
  },
  constellations: 10,
  upgrades: 34,
