...
Living HD menu scenery (2026-09-07): `menuAtmosphere.js` selects v4 at full opacity without screen dimming. `menuAtmosphereLiving.json` owns generation and per-scene export timing. The six 1080p loops play 15-32% slower than v3; each selected scene repeats into itself. `menuAtmosphereMotion.json` retains the historical v2 build profiles.
