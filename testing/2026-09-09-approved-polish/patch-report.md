...
- Death records/saves still happen immediately. Presentation performs a bounded visual fall to a nearby floor if airborne, plays the existing 73-frame Survivor collapse, holds the last pose for 450 ms, then fades in the recap over 420 ms. Shutdown cancels the presentation. Missing animation/event fallbacks avoid trapping the player. The death sheet is prewarmed and retained.
