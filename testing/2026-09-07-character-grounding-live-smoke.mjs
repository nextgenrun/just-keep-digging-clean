...
const output = process.env.CHARACTER_ALIGNMENT_OUTPUT || "testing/character-grounding-2026-09-07/after";
...
async function waitForGait(key,speed) {
  await page.waitForFunction(({key,speed})=>{
    ...
    const c=s.playerController;
    return Math.sign(c.physicsBody.vx)===Math.sign(speed) && Math.abs(c.physicsBody.vx)>40 && c.isRunning()===(Math.abs(speed)>200);
...
async function exercise(key,name) {
  ...
  await page.evaluate(key=>{
    ...
    if(s.__alignmentSampler)s.events.off('postupdate',s.__alignmentSampler);
    s.__alignmentSampler=(time,delta)=>{
      const c=s.player,b=s.playerController.physicsBody;
      if(s.__alignmentSamples.length<3000)s.__alignmentSamples.push({pose:c.anims.currentAnim?.key, x:c.x, y:c.y, t:time, delta, measuredSpeed:s.playerKinematicMotion?._frameSpeedX, vx:b.vx, vy:b.vy, grounded:s.playerController.isGrounded(), flip:c.flipX,
        timeScale:c.anims.timeScale, shadowX:s.playerContactShadow?.outer?.x, shadowY:s.playerContactShadow?.outer?.y,
        shadowAlpha:s.playerContactShadow?.outer?.alpha,
    ...
    s.events.on('postupdate',s.__alignmentSampler);
  },key);
  await recordStart();
  const idle=await read(key);
  await page.keyboard.down('d');await waitForGait(key,160);await page.waitForTimeout(1700);
  ...
  await page.keyboard.down('Control');await waitForGait(key,336);await page.waitForTimeout(1700);
  ...
  await page.keyboard.down('d');await page.keyboard.down('Control');await waitForGait(key,336);await page.waitForTimeout(350);
  await page.keyboard.up('d');await page.keyboard.down('a');await waitForGait(key,-336);await page.waitForTimeout(400);
  await page.keyboard.up('a');await page.keyboard.up('Control');await page.waitForTimeout(450);
...
try {
  await page.goto("http://127.0.0.1:8098/?jkd_e2e=1&cinematics=0&character=survivalUal"+(process.env.CHARACTER_GROUNDING_BEFORE ? "&characterGrounding=0" : ""),{waitUntil:"domcontentloaded",timeout:60000});
  ...
  if(process.env.CHARACTER_CAPTURE_ONLY || process.env.CHARACTER_MATCHED_CAPTURE) await page.evaluate(()=>{
    const s=window.__phaserGame.scene.getScene('PlayScene');
    s.dayNightCycle.currentTime=0.37;s.dayNightCycle.dayDuration=1e12;
    s.weatherSystem.forceWeather('clear',0,600000);
  });
  ...
  if(!process.env.CHARACTER_CAPTURE_ONLY) {
  ...
  results.rendering=await page.evaluate(()=>{
    const s=window.__phaserGame.scene.getScene('PlayScene'),gl=s.game.renderer.gl;
    return {density:s.game.__jkdRenderDensityProfile, appearance:s.playerWorldAppearance?.active, characterPostPipelines:s.player.postPipelines.length,
      grounded:!!s.playerContactShadow?.groundedShadow,
      mipmaps:(s.playerAssetProfile.mipmappedCharacterSheets||[]).filter(k=>s.textures.exists(k)).map(k=>({key:k,filters:s.textures.get(k).source.map(v=>v.glTexture.minFilter)})),
      glError:gl.getError(), contacts:s.groundFootstepFxSystem.contactSequence};
  });
  assert.equal(results.rendering.appearance,true,'Neutral world shading is active');
  assert.equal(results.rendering.characterPostPipelines,0,'Shading requires no extra render pass');
  assert.equal(results.rendering.glError,0,'Character texture sampling has no WebGL error');
  // Exercise the complete held-torch gait and stop variants through actual input.
  await page.evaluate(()=>window.__phaserGame.scene.getScene('PlayScene').playerController.teleportToTile(12,64));
  await page.waitForFunction(()=>window.__phaserGame.scene.getScene('PlayScene').playerController.isGrounded());
  await page.keyboard.press('t');await page.waitForTimeout(200);
  await exercise('PlayScene','held-torch-walking');
  ...
  results.failure=await read("PlayScene").catch(()=>null);
