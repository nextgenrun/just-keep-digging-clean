...
export class CelestialTalentTreeView {
  ...
  _build() {
    ...
    this.talentPointsText = this._text(this._x(layout.talentPointsXFraction), this._y(layout.headerYFraction), "",
      presentation.currencyFontSizePx, presentation.titleColor);
    this.starsText = this._text(this._x(layout.starsXFraction), this._y(layout.headerYFraction), "",
      presentation.currencyFontSizePx, presentation.readyColor);
    ...
    ).setWordWrapWidth(layout.detailTitleWidthPx);
    ...
    ).setWordWrapWidth(layout.detailStatusWidthPx);
  ...
  refresh() {
    ...
    this.talentPointsText.setText(`${this.config.copy.talentPointsTitle}\n${snapshot.talentPoints}`);
    this.starsText.setText(`${this.config.copy.starPointsTitle}\n${snapshot.stars.toLocaleString("en-US")}`);
    ...
    this.connectorLayer?.refresh(
      this.nodesById,
      this.nodes[this.selectedIndex]?.node.id || null,
    );
    this._refreshDetail();
  }
  ...
  purchaseNode(nodeId = this.nodes[this.selectedIndex]?.node.id) {
    ...
    const action = this.progression?.getNodeAvailability?.(nodeId)?.action;
    const result = (action === "upgrade"
      ? this.progression?.upgradeNode?.(nodeId)
  ...
  resize() {
    ...
    const detailScale = scale < layout.detailMinimumScreenScale
      ? layout.detailMinimumScreenScale / scale : 1;
    [this.detailTitle, this.detailBody, this.detailStatus]
      .forEach(text => text.setScale(detailScale));
    const compact = scale < layout.compactStatusScaleThreshold;
    this.nodes.forEach(node => {
      node.setViewportScale(scale);
      node.setCompactStatus(compact);
    });
  }
  ...
  getHealthSnapshot() {
    ...
    return Object.freeze({
      ...
      connectorStates: this.connectorLayer?.getStateSnapshot?.() || null,
