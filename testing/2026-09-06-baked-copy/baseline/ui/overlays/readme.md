...
- `CelestialTalentTreeView.js`, `CelestialTalentTreeNodeView.js`, and
  ...
  the view reads the live row/lane graph instead of hard-coding the retired
  five-node columns. Level 3 opens the tree, Talent Points unlock nodes, and
  Star Points raise owned nodes through three visible ranks. Future nodes stay
  visually quiet; `1 TP`, `NEXT LEVEL`, and `FINISH PATH` identify only the
  next useful action. Locked, ready, owned, and selected routes have separate
  connector emphasis, and compact layouts preserve physical text size. V2 uses a socket-free cobalt
