...
- `downwardDigAnimations.js` owns the default-on DOWN extension. It appends the
  reviewed low body punch and leg sweep to the retained ground strike, pins the
...
for the ten-clip complex SIDE asset catalog; live selection uses the nine
approved stages and retains the excluded Cross aliases for rollback/audit.
...
persistent notice geometry, local developer controls and F2. Review-only
chamber conditions stay in `dynamicEventReview.js`.
