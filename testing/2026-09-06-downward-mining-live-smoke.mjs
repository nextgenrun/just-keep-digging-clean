...
const output = "testing/2026-09-06-downward-mining-proof";
await fs.mkdir(output, { recursive: true });
// Use this checkout's canonical server with a drained log and isolated port.
...
const server = spawn("C:/Users/Mila/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/python.exe",
  ["-u","-c","import runpy,sys,webbrowser; webbrowser.open=lambda *a,**k: False; sys.argv=['serve.py','8096']; runpy.run_path('serve.py',run_name='__main__')"],
...
async function capture(sceneKey, name, seconds = 7) {
  await page.evaluate(key => {
    ...
    const contact = e => trace.contacts.push({ t:s.time.now, key:e.animationKey,
      frame:e.contactFrame,visibleFrame:e.visibleFrame, index:e.contactIndex });
...
try {
  await page.goto("http://127.0.0.1:8096/?jkd_e2e=1&cinematics=0&character=survivalUal",{waitUntil:"domcontentloaded",timeout:60000});
  ...
  if(!baseline) {
    await page.evaluate(()=>{
      const s=window.__phaserGame.scene.getScene("PlayScene"),z=s.worldModel.caveZones.find(z=>z.entry);
      if(!z)throw new Error("No existing cave fixture");
      s.caveEntryController.enter(z,z.entry);
    });
    ...
    const caveFixture = await stage("CaveScene");
    await page.evaluate(f=>{window.__downFixture=f},caveFixture);
    console.log("CAVE_FIXTURE",JSON.stringify(caveFixture));
    await page.waitForFunction(()=>window.__phaserGame.scene.getScene("CaveScene").playerController.isGrounded());
    await capture("CaveScene","cave-down");
  }
  ...
  await browser.close();
  server.kill(); closeSync(serverLog);
}
