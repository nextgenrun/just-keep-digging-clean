...
const output = path.join(root, 'visual-approval-previews/2026-09-05-most-seen-visuals');
...
const server = spawn('C:/Users/Mila/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/python.exe', [
  '-u', '-c', "import sys,runpy,webbrowser; webbrowser.open=lambda *a,**k: False; sys.argv=['serve.py','8193']; runpy.run_path('serve.py',run_name='__main__')",
...
try {
  ...
  browserServer = await chromium.launchServer({
    ...
    args: ['--autoplay-policy=no-user-gesture-required', '--remote-debugging-port=9337'],
  });
  ...
  await fs.writeFile(path.join(output, 'session.json'), JSON.stringify({ endpoint: browserServer.wsEndpoint(), origin, serverPid: server.pid }));
  ...
  await page.screenshot({ path: path.join(output, '00-current-menu.png') });
  ...
  await page.evaluate(() => {
    ...
    window.__phaserGame.scene.start('WorldLoadScene', {
      saveSlot: 1, worldIdentity: 'most-seen-visual-review-20260905', isNewSave: true, tutorialChoice: 'no',
    });
  });
  ...
  await page.screenshot({ path: path.join(output, '01-current-start.png') });
  await fs.writeFile(path.join(output, 'initial-state.json'), JSON.stringify(await page.evaluate(() => ({
  ...
  await fs.writeFile(path.join(output, 'session-errors.json'), JSON.stringify(errors, null, 2));
} finally {
