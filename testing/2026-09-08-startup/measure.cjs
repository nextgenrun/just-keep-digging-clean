...
(async()=>{
 ...
 const tag=play?'optimized-world-smoke':production?'production-recovery-7mbps':remote?'live':fail?'recovery-7mbps':slow?'optimized-7mbps':'baseline';
 ...
 const errors=[],previewApiWarnings=[],requests=[],responses=[];let bytes=0,firstLoading=null,last=null,bootAttempt=0;
 ...
 try{
 const nav=await p.goto(remote?'https://www.nextgen.run/diggame-beta-1/':production?JSON.parse(fs.readFileSync(path.join(__dirname,'preview-server.json'),'utf8').replace(/^\uFEFF/,'' )).url+'/index.html?startupQa='+tag:'http://localhost:8765/index.html?jkd_e2e=1&startupQa='+tag,{waitUntil:'commit',timeout:60000});console.log('NAV',nav.status());
 for(let i=0;i<(remote?90:180);i++){
  last=await p.evaluate(()=>{const g=window.__phaserGame,s=g?.scene?.getScene('BootScene');return {scenes:g?.scene?.getScenes(true).map(s=>s.sys.settings.key)||[],loading:!!s?.loadingUi,progress:s?.load?.progress,queue:s?.load?.totalToLoad,failed:s?.load?.totalFailed,logo:s?.children?.getByName('brand-logo-view')?.getData('state'),title:document.title}});
 ...
 const report={tag,gameplay,manualRetried,previewApiWarnings,injectedFailures,failedAssetRequests,bootAttempt,seconds:(Date.now()-start)/1000,firstLoading,bytes,last,errors,files,responses};fs.writeFileSync(path.join(__dirname,tag+'.json'),JSON.stringify(report,null,2));if(!remote&&(!last?.scenes.includes('MainMenuScene')||errors.length))process.exitCode=1;if(fail&&(injectedFailures!==failureLimit||failedAssetRequests<failureLimit+1||bootAttempt<(manual?4:2)||(manual&&!manualRetried)))process.exitCode=1;console.log('DONE',JSON.stringify({...report,files:files.slice(0,15),responses:responses.slice(0,4)}));await b.close();
