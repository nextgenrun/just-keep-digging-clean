...
check("Current review keeps source IDs, with matching trimmed preview metadata", () => {
 ...
 assert.deepEqual(current.items.map(i => [i.id, i.path]), before.items.map(i => [i.id, i.path]));
 const changed = current.items.filter(i => i.metadata.playbackWindow); assert.equal(changed.length, Object.keys(CORE_SFX_WINDOWS).length);
