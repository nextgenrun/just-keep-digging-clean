...
document.getElementById("check").onclick = async () => {
 ...
 try {
  ...
  report.landings = [];
  for (const tileType of [TILE_TYPES.STONE, TILE_TYPES.DIRT]) {
   const controller = { physicsBody: { x: 0, y: 14, w: 40, h: 80, vx: 0 }, isGrounded: () => true };
   const world = { tileSize: 94, getTileType: () => tileType };
   const sound = system.playLanding(controller, REVIEWED_AUDIO_MIX.landing.fullSpeed, world);
   assert(sound?.source?.buffer, "Landing must have one native source");
   assert(sound.audioBuffer.duration <= 0.121, "A long landing tail returned");
   assert(!["approved-review-libDirtBreak", "approved-review-libLandingDebris"].includes(sound.key), "Destruction returned to landing");
   const row = { tileType, key: sound.key, bufferMs: sound.audioBuffer.duration * 1000, outputGain: sound.currentConfig.volume * system.masterVolume };
   await new Promise((resolve, reject) => {
    const deadline = setTimeout(() => reject(new Error("Landing did not complete")), 1000);
    sound.once("complete", () => { clearTimeout(deadline); resolve(); });
   });
   assert(system.playFootstep({ controller, worldModel: world, landingGain: 1 }) === null, "Landing stacked a second ground contact");
   row.completed = true; report.landings.push(row);
  }
  ...
  report.delayedXpSilent = true; report.passed = true; state.textContent = "PASS — 11 segments, 2 landings and delayed XP checked";
