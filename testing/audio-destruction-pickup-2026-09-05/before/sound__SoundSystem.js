...
export class SoundSystem {
  ...
  playXpGather({ special = false, levelUp = false } = {}) {
    // Routine XP is already visible and accompanies the resource arrival.
    // A level-up has its own approved reward cue; do not add another UI tick.
    if (!special || levelUp) return null;
    return this.playResourcePickup({ special: true });
  }
  ...
  playFootstep({ controller = this.scene.playerController, worldModel = this.scene.worldModel, landingGain = null } = {}) {
    ...
    const fallback = () => {
      ...
      return asset ? this.playSfx(key, asset.gain * (landingGain ?? 1), {
        rawGain: true, sourcePeak: asset.peak,
      }) : null;
    };
  ...
  playLanding(controller, speed, worldModel = this.scene.worldModel) {
    ...
    if (sound) { this.footstepOwner = controller; this.lastFootstepTime = this.freesoundAudio.now(); }
    return sound;
  }
