...
test("every standing combo and running phase has its own actual contact-frame record", () => {
  ...
  for (const key of keys) {
    ...
    for (const contact of spec.contacts || [spec]) {
      ...
      assert.ok(Object.values((CHARACTER_DEFINITION_IMPACT_CONTACTS[variant.sheet] || DIG_IMPACT_CONTACTS[variant.sheet]).contacts)
...
test("unified timing fixes retain each clip and both authored combo hits", () => {
  ...
  for (const key of downPhases) assert.equal(resolveUalActionContact(PROFILE, key).sequenceIndex, 12);
});
...
test("running composites project their real 192 px geometry at the existing 117 px size", () => {
  ...
  const row = Object.values((CHARACTER_DEFINITION_IMPACT_CONTACTS[variant.sheet] || DIG_IMPACT_CONTACTS[variant.sheet]).contacts).find(r => r[0] === spec.textureFrame);
  assert.equal(hit.authored, true);
