...
export class SoundSystem {
  ...
  playDigSwing() { return this.reviewedSfx.play("libDirtSwingA"); }
  ...
  playFootstep({ controller = this.scene.playerController, worldModel = this.scene.worldModel } = {}) {
    if (!this.sfxEnabled || !this.audioInitialized) return null;
    const fallback = () => {
      const soundKey = this.soundLibraryManager.getRandomSound('footsteps');
      return soundKey && this.soundLibraryManager.soundExists(soundKey) ? this.playSfx(soundKey) : null;
    };
    const body = controller?.physicsBody;
    const size = worldModel?.tileSize || this.scene.config?.tileSize;
    const floor = body && size ? worldModel?.getTileType?.(
      Math.floor((body.x + (body.width || 0) / 2) / size),
      Math.floor((body.y + (body.height || 0)) / size + this.freesoundAudio.config.floorProbeTiles),
    ) : null;
    return this.freesoundAudio.config.softTypes.includes(floor)
      ? this.freesoundAudio.play("footstepDirt", {}, fallback) : fallback();
  }

  playDig(options = {}) {
    this.freesoundAudio.noteAction();
    const id = this.reviewedSfx.choose("dig", ["digOne", "digTwo"]);
    const body = this.freesoundAudio.play(this.freesoundAudio.materialRole(options.tileType),
      { ...options, cooldownKey: "dig-contact" }, () => this.reviewedSfx.play(id, { ...options, cooldownKey: "dig-contact" }));
    if (body && !REVIEWED_AUDIO_MIX.softMaterialTypes.includes(options.tileType)) {
      if (this.freesoundAudio.isMetal(options.tileType)) {
        this.freesoundAudio.play("mineMetal", options, () => this.reviewedSfx.play("libToolContact", options));
      } else this.reviewedSfx.play("libToolContact", options);
    }
    return body;
  }

  playTileBreak(options = {}) {
    const id = REVIEWED_AUDIO_MIX.softMaterialTypes.includes(options.tileType)
      ? "libDirtBreak" : "libStoneBreak";
    const fallback = () => this.reviewedSfx.play(id, { ...options, gain: Math.min(1.2, options.volume ?? 1) });
    return this.freesoundAudio.isCrystal(options.tileType)
      ? this.freesoundAudio.play("crystalBreak", options, fallback) : fallback();
  }
