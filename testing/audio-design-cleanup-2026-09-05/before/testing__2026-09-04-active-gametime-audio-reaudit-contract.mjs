...
assert.ok(fresh.items.length > 850, "active catalog unexpectedly small");
assert.equal(fresh.items.filter(item => item.routes.some(route => route.source === "freesound-runtime")).length, 562);
...
assert.ok(fresh.items.filter(item => item.suggestedGain < item.runtimeGain).length > 900,
