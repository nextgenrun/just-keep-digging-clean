...
export class ReviewedAmbienceController {
  ...
  observeMotion(controller, delta = 16) {
    const body = controller?.physicsBody;
    if (!body) return;
    const next = { x: body.x, y: body.y, grounded: controller.isGrounded?.() === true,
      speed: Math.max(0, body.vy || 0), airMs: 0 };
    const previous = this.motion;
    this.motion = next;
    if (!previous || Math.hypot(next.x - previous.x, next.y - previous.y) > MIX.landing.teleportDistance) return;
    if (!next.grounded) {
      next.airMs = (previous.grounded ? 0 : previous.airMs) + Math.min(100, delta);
      next.speed = Math.max(next.speed, previous.grounded ? 0 : previous.speed);
    } else if (!previous.grounded && previous.airMs >= MIX.landing.minAirMs && previous.speed >= MIX.landing.minSpeed) {
      this.system.reviewedSfx.play("libLandingDebris", { gain: Math.max(0.3, clamp(previous.speed / MIX.landing.fullSpeed)) });
    }
  }
