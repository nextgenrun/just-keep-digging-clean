...
const oldCatalog = JSON.parse(readFileSync(new URL("before/testing__audio-runtime-reaudit-2026-09-04__catalog.json", import.meta.url)));
const hardSteps = oldCatalog.items.filter(item => item.routes.some(route => route.source === "legacy-runtime" && route.role === "footsteps"))
 .map(item => ({ key: item.routes[0].key, path: item.path, duration: metrics[item.path].duration }));
...
function capture(id, title, run) {
 ...
 f.scene.sound.add = (key, options) => {
  ...
  sound.play = (...args) => {
   ...
   const event = { key, path: asset?.path, title: asset?.title || asset?.id || key,
    ...
    rate: options.rate || 1, duration: metrics[asset.path].duration, gains: [] };
 ...
 f.scene.playerController = { physicsBody: { x: 0, y: 0, width: 40, height: 80, vx: 130, vy: 0 },
