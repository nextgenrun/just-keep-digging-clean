...
export class ThunderStrikeActionRuntime {
  ...
  update(time, thunderPressed = false, onStrikeResult = null) {
    ...
    if (this.animating) {
      ...
      const snapshot = this.state.phase === THUNDER_STRIKE_CHAIN_PHASES.TIMING
  ...
  _updateActive(nowMs, thunderPressed, onStrikeResult) {
    ...
    if (this.state.phase === THUNDER_STRIKE_CHAIN_PHASES.TIMING) {
      if (thunderPressed) {
        const attempt = this.state.attemptContinuation(nowMs);
        if (!attempt.success) {
          this._breakChain(nowMs, attempt.snapshot);
          return;
        }
        const armed = abilities.armThunderStrikeFollowUp?.(attempt.stageIndex) === true;
        if (!armed) {
          this._breakChain(nowMs, attempt.snapshot);
          return;
        }
