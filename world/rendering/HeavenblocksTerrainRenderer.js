...
export class HeavenblocksTerrainRenderer {
  ...
  _createRecord(tx, ty, type, textureKey, damageStage, signature, tint) {
    ...
    const displayScale = isRelic ? 1.28 : isBarrier ? 1.12 : 1.045;
    const depth = isRelic
    ...
    if (isRelic || isBarrier) {
      record.aura = this.scene.add.image(x, y, textureKey)
        .setDisplaySize(size * (isRelic ? 1.62 : 1.3), size * (isRelic ? 1.62 : 1.3))
        .setDepth(depth - 0.01)
        .setTint(tint)
      ...
      record.tween = this.scene.tweens.add({
        ...
        scaleX: isRelic ? 1.08 : 1.04,
        scaleY: isRelic ? 1.08 : 1.04,
  ...
  _resolveRegion(tx, ty, type) {
    ...
    if (type !== TILE_TYPES.ANCIENT_RELIC_CACHE) return null;
    const regions = Object.keys(this.config.biomes);
    const regionId = tx < 63 ? regions[0] : tx < 120 ? regions[1] : regions[2];
    return { id: regionId, baseTileType: -1, oreTileType: -1 };
  }
