...
export function addResourceCodexText(scene, parent, x, y, value, style = {}) {
...
export function addResourceCodexHitZone(
  scene,
  parent,
  x,
  y,
...
export function hasDiscoveredResource(scene, items, resourceKey) {
  return Number(items?.[resourceKey]) > 0
    || scene.retentionProgressSystem?.hasDiscoveredMaterial?.(resourceKey) === true;
}

export function getResourceCodexMaterialClass(resourceKey) {
  if (["dirt", "darkDirtNormal", "darkDirtStrong", "lavaDirt"]
    .includes(resourceKey)) return "GROUND";
  if (resourceKey === "magmaCrystal") return "CRYSTAL";
  return "ORE";
}
