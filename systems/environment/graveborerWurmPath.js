...
export function resolveGraveborerWurmCollision(renderState, playerTile, config) {
  if (!copyGraveborerTile(playerTile)) return null;
  const candidates = [renderState.head, ...renderState.bodies]
    .filter(part => part?.visible);
  for (const part of candidates) {
    const radius = part.kind === "head"
      ? config.combat.headHitRadiusTiles
      : config.combat.bodyHitRadiusTiles;
    if (Math.hypot(part.x - playerTile.tx, part.y - playerTile.ty) <= radius) {
      return {
        part: part.kind,
