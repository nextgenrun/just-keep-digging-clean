...
export class LevelUpRewardPresentation {
  constructor(scene) {
    ...
    this.active = scene.textures?.exists?.(ASSET_KEYS.ui.approvedHud.notification) === true;
    ...
    this.root = scene.add.container(0, 0)
      ...
      .setVisible(false);
    this.background = scene.add.image(0, 0, ASSET_KEYS.ui.approvedHud.notification)
    ...
    this.darkness = this._createText(
      this.config.darknessY,
      this.config.darknessFontSize,
      this.config.colors.darkness,
    );
    ...
    this.root.add([this.background, this.title, this.darkness, this.reward]);
    this.resize();
  }
  ...
  show(reward = {}) {
    ...
    this.title.setText(levelsGained > 1
      ? `${copy.level} ${level}${copy.separator}+${levelsGained} ${copy.levels}`
      : `${copy.level} ${level} ${copy.reached}`);
    this.darkness.setText(
      `${copy.darknessResistance} +${Math.max(0, Math.round(reward.darknessResistanceGainMeters || 0))}m`
      + `${copy.separator}${copy.total} ${Math.max(0, Math.round(reward.darknessResistanceMeters || 0))}m`,
    );
    this.reward.setText(
  ...
  getHealthSnapshot() {
    return {
      ...
      darknessResistanceMeters: this.lastReward?.darknessResistanceMeters || 0,
