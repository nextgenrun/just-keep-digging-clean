...
export function unifiedFootstepSheet(profile, player, frame) {
  ...
  const key = frame?.textureKey || player?.texture?.key;
  return profile.groundingContacts?.[key] || PLAYER_FOOTSTEP_CONTACTS.sheets[key] || null;
}
...
export function groundFootstepAnchor({ player, body, profile, manifest, frame, config, polished }) {
  ...
  if (sheet && (sheet.size !== width || sheet.size !== height)) {
    if (!profile.characterDefinitionRuntime || width !== height) return fallback;
    marker = marker.map(value => value * width / sheet.size);
  }
