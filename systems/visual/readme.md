...
- `TitanDiscoverySystem.js` / `TitanSurfaceGallery.js` — track 25 deterministic clear-area windows without mutating tiles, reveal silhouettes behind authoritative terrain in both scenic and legacy renderer modes, play a visual-only glow/echo unlock, persist canonical discovered ids through retention data, maintain all 25 generated-art plinths in the non-interactive surface Titan Walk, and publish zone/slot/art readiness through `__jkdTitanDiscoveries` plus the runtime canary (`values/titanDiscoveries.js`, rollback `?titans=0`)
