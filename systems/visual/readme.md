...
- `TitanDiscoverySystem.js` / `TitanChamberStream.js` / `TitanSurfaceGallery.js` — track 25 deterministic colossal clear-area windows without mutating tiles, stream at most two unique 1536x848 chamber cards near the player, keep compact art as a live fallback, reveal the cards behind authoritative terrain in both renderer modes, play the visual-only glow/echo unlock, persist canonical ids through retention, pin one discovered archive vignette on demand, maintain all 25 generated-art plinths in the surface Titan Walk, and publish zone/slot/stream/art readiness through `__jkdTitanDiscoveries` plus the runtime canary (`values/titanDiscoveries.js`; `?titanChambers=0` art rollback; `?titans=0` complete rollback)
