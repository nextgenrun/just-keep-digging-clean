...
- `FloatingTextSystem.js` — policy-gated world text + constellation UI progress;
  ...
  delegate their transient presentation to `SkyStarReleaseView.js`; their saved
  constellation progress and callbacks remain authoritative without a pickup
  card
- `SkyStarReleaseView.js` — ImageGen-only mined Star Block release: the
  tile-matched crystal core fades in below full alpha, reaches the exact 94 px
  live-tile envelope before growth, then rises to a restrained 136 px peak and
  levitates for at least 10.8 seconds behind six
  paced authored-image echoes. Matching rarity fracture/pulse art remains
  beneath it; the view never draws circles/graphics, tints art, or generates
  textures
...
- `HardcoreMemorialWorldSystem.js` — plants the approved ImageGen grave at each
  exact same-slot Hardcore death position in later Casual or Hardcore runs.
  Nearby graves receive bounded lateral separation, remain below the player,
  and expose the recorded depth/cause through the approved notification system.
...
- `CaveLevelPresentationSystem.js` — gives entered caves a 60x20-tile,
  camera-traversed ImageGen panorama, routes six cave identities across three
  authored visual families, and hides only the unbreakable structural collision
  cells so the painted floor and walls remain the visible environment.
...
- `HeavenblocksArtifactSystem.js` / `HeavenblocksPortalVisualSystem.js` /
  `HeavenblocksArtifactFx.js` — place modular biome props, shrines, installed
  parts, vaults, authored portal sockets, restored ground routes, and bounded
  unlock/arrival/relic/component/vault/forge effects over real `WorldModel`
  geometry. Display-size pulses preserve each image's configured tile-space
  scale; none of these systems owns collision or progression state.
