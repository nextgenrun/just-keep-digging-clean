...
- `npcActivityConfig.js` owns the approved v8 merchant activity cadence,
  anchored pacing limits, pose timing, preload paths, query rollback, and
...
- `undergroundBiomeMotionReview.js` owns only the ten review-gallery motion
  identities, source-plate paths, accent pairs, material ids, and loop values.
  It is not imported by Phaser and cannot promote an unapproved mockup.

- `titanDiscoveries.js` owns the 25 visual-only titan identities, transparent sprite paths, deterministic depth/x search anchors, clear-area thresholds, backdrop reveal/glow timing, and the non-interactive surface-echo procession. `?titans=0` disables the complete presentation without changing terrain, rewards, collision, or saves.

- `worldVisualSurfacePacks.js` owns the reversible scenic-surface composition. `town-benchmark-v1` is the default: its approved 1801x941 beauty plate is cropped at the authored ground line and uniformly calibrated from the shared 1.75 m midpoint player reference so its measured 75 px lintel-to-threshold opening renders as a 2.10 m door. This produces a roughly 23.05-tile beauty span with a bounded 20.3% enlargement and no aspect distortion. Approved Option A adds a separate 2172x139 transparent slate-and-fieldstone Town Square facade at essentially native density; it shares the authoritative solid-terrain mask and sits above the generic bedrock material while remaining below roots and damage feedback. The legacy 14x10 ground facade remains underneath for exact digging holes and rollback continuity. The pack also owns restrained SCREEN lightning plus cool, low-alpha wet-ground response. Use `?surfacePack=current-v2` to roll back to the previous split scenic surface assembly without changing gameplay state.
...
- `worldDepthConfig.js` owns the 2,000-row Level 1 boundary and 5,000-meter Level 2 runtime depth; `arcCoreConfig.js` owns both Arc Core material costs, the 2x2 base footprint, and the final 8x8 Omega footprint. `craftingRecipes.js` owns their permanent relic, Heavenblock, part, and Zenith requirements plus Forge presentation copy. `arcCoreAnimationReview.js` owns the review-only small/Omega motion language and explicitly cannot promote either mockup into production. `arcCoreReview.sprite.json` is the centralized Phaser asset pack for the drift-locked review art: it owns all six body/VFX roles, paths, hashes, pivots, display sizes, depths, and layered-motion values.
