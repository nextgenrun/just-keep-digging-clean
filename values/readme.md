...
- `gameplayDevFlags.js` owns the active gameplay profile. `demoMode` is currently `true`, excluding Level Two/the tunnel gate, Arc Core and Omega Arc Core, developer cheats, and screen capture through shared upgrade, level, keybind, and feature admission helpers.
